...
URL="https://movie.douban.com/top250" #要爬取的地址
iplist=['218.86.128.100:8118','','','']
proxies = {'http' : random.choice(iplist)}
HEADERS = {
        'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.80 Safari/537.36'
}#user-agent，模仿浏览器，防止被目标网站反爬
def download_page(url):#下载页面
    data = requests.session().get(url, proxies=proxies,headers=HEADERS, timeout=10).content #请求页面，获取要爬取的页面内容
    return data
def parse_html(html):
